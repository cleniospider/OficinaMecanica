...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['nome'])) {
    ...
    if (!empty($nome) && $preco_venda >= 0 && $estoque_atual !== false && $estoque_minimo !== false) {
        try {
            ...
            // Decidir pra qual tela de estoque voltar baseado no novo estoque
            if ($estoque_atual <= 2) {
                header("Location: estoque-critico.php");
            } elseif ($estoque_atual < $estoque_minimo) {
                header("Location: estoque-baixo.php");
            } else {
                header("Location: estoque-ok.php");
            }
            exit;
...
<head>
    ...
    <title>Auto Repair - Editar Peça</title>
