<?php 
session_start(); // Inicializa a sessão para ler o perfil do usuário logado e evitar erros
...
try {
    $stmt = $pdo->query("SELECT * FROM pecas WHERE estoque_atual <= 2 ORDER BY nome ASC");
    $pecas = $stmt->fetchAll();
} catch (PDOException $e) {
    die("Erro ao carregar estoque crítico: " . $e->getMessage());
}
?>
...
<head>
    ...
    <title>Auto Repair - Estoque Crítico (Mecânico)</title>
            ...
            <div class="estoque-wrapper">
                <div class="estoque-controles">
                    <div class="filtros-grupo">
                        <a href="estoque-critico-mecan.php" class="btn-f b-critico ativo">CRÍTICO</a>
                        <a href="estoque-baixo-mecan.php" class="btn-f b-baixo">BAIXO</a>
                ...
                <div class="lista-pecas" id="listaPecasContainer">
                    <?php if (empty($pecas)): ?>
                        <p style="text-align: center; color: #aaa; width: 100%; margin-top: 20px;">Nenhuma peça no estoque crítico.</p>
                    <?php else: ?>
