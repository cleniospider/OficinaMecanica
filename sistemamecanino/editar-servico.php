...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['nome'])) {
    ...
    if (!empty($nome) && $preco >= 0) {
        try {
            $stmt_up = $pdo->prepare("UPDATE servicos SET nome = ?, preco = ? WHERE idservicos = ?");
            $stmt_up->execute([$nome, $preco, $id]);
            header("Location: servicos.php");
            exit;
...
<head>
    ...
    <title>Auto Repair - Editar Serviço</title>
...
<body>
    ...
    <main class="main-content">
        <div class="container-form-dark">
            <h2 class="titulo-sessao">EDITAR SERVIÇO</h2>
                ...
                <form method="POST">
                    ...
                    <div class="form-row">
                        ...
                        <div class="grupo-input-dark flex-1">
                            <label>PREÇO</label>
                            <input type="text" id="preco" name="preco" value="R$ <?= number_format($servico['preco'], 2, ',', '.') ?>" required>
                        </div>
                    </div>
